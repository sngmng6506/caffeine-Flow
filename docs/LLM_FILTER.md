...
- 사람 라벨은 `recommendations.filter_status`와 큐 `status`를 바꾸지 않는다. 음악 라벨링은 곡 버전과 메타데이터 충분성을 묻지 않고 기존 값은 보존하며 신규 값은 각각 `unknown`, `false`를 사용한다. AI 승인·거절 결과는 판단 당시 매장 정책 아래에 표시한다.
