...
| 템포 | 수동 CLI로 분석한 행에만 있다 |

신청 시점 분석(`EMOTION_LLM`)에는 장르 후보와 템포가 없고, 값이 없는 줄은 프롬프트에서 빠진다. 형태는 `track-analysis.js`와 `prompts/music-filter.user.njk`가 기준이다.
...
- 사장님이 안내를 직접 고칠 수 있다(`PUT /cafes/me/music-filter/public-notice`). 저장하면 소켓으로 손님 화면에 바로 반영된다. 직접 쓴 문구도 같은 길이 상한을 서버가 확인한다.
- 직접 고친 문구는 매장 분위기 설명을 바꿔 저장하면 AI 문구로 덮인다. 정책이 바뀌었는데 옛 정책을 설명하는 안내가 남는 것을 막기 위해서다.
...
- 사람 라벨은 `usage_scope=operational`이어도 라이브 판단·자동수락·Exact 재사용에 연결하지 않으며 성능 지표도 자동 계산하지 않는다. 동일 아티스트 참고 조회는 운영자 수동 조회만 제공한다. 라이브 연결은 별도 평가와 계약 변경 후 진행한다.
