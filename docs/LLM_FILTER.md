...
`filter_prompt_snapshot`은 해당 판단에 실제 사용한 매장 분위기 설명을 보존한다. 설정 변경 시각으로 과거 프롬프트를 추정하지 않는다. 감사 기능 도입 전 판단은 정확한 값을 알 수 없으므로 `null`로 유지한다.

설정 변경은 `music_filter_prompt_history`에 현재 상태를 기록한다. 마이그레이션 시점의 기존 설정은 실제 변경 시각으로 오해하지 않도록 `record_type=baseline`으로 구분하며, 이후 변경은 `changed`로 기록한다. 같은 값을 다시 적용할 때는 중복 이력을 만들지 않는다.
...
운영자는 관리자 콘솔의 카페별 AI 판단 이력 또는 `/labeling-lab`의 전체 카페 통합 큐에서 다음 골드 라벨을 기록한다.
