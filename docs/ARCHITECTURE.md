...
| `audio-analysis-worker/` | 로컬 음원의 Essentia 특징 추출, 분류 헤드 기반 라벨 추천과 신뢰도 산출, 결과 제출. 디렉터리 큐 기반 상주 워커이며 원본 전송 없음 |
...
| `music_audio_analyses` | 곡·모델 버전별 자동 음향 특징, 추천 라벨과 검수 상태 |
