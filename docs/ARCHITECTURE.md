...
| `admin/` | 플랫폼 운영자용 카페 모니터링·정지·삭제와 AI 랩 진입 UI |
| `music-filter-lab/` | 관리자 세션으로 모델·프롬프트별 필터 판단을 저장 없이 비교하는 UI |
| `music-labeling-lab/` | 전체 카페 AI 처리 이력을 한 큐에서 골드 라벨링하는 UI |
...
| `music_track_annotations` | 플랫폼 원본 곡별 수동 음악 특성 라벨. 정규화한 아티스트 키로 다른 곡 참고 조회 |
