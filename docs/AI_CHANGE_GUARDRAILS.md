...
- **신청 시점 경로는 `EMOTION_LLM`이다** — Valence/Arousal + Audio LLM. MAEST를 다시 넣지 않는다(지연 근거는 `git log -S EMOTION_LLM`). `MAEST_ONLY`·`MAEST_EMOTION`·`FULL`은 이미 저장된 행을 읽을 때만 쓴다.
...
- **3단 프롬프트에 MAEST 태그 점수·임베딩 벡터·고정 택소노미를 넣지 않는다.** 점수는 보정되지 않은 상대값이라 모델이 곡끼리 비교하게 되고, 택소노미는 학습 분포 밖 음악의 정보를 지운다. **Valence/Arousal만 예외다** — `normalize_score`가 DEAM `[1,9]`를 `[0,1]`로 옮긴 보정된 값이라 0.6이 어디서나 같은 뜻이다. 3단과 필터 프롬프트 모두 척도를 함께 적어 숫자로 넣고 밴드 이름으로 바꾸지 않는다 — 밴드로 뭉치면 가까운 값의 차이가 사라진다. 두 곳의 형식(`0.60 (0.00 어두움 ~ 1.00 밝음)`)을 같이 유지한다.
