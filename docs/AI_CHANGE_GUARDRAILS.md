...
- 3단 Audio LLM 프롬프트에 MAEST 결과·임베딩 벡터·고정 택소노미를 넣지 않는다. 두 단계는 독립이어야 앙상블이 되고, 결과가 갈리는 곡이 어려운 곡이라는 신호가 남는다. `audio_llm_raw`는 자유 서술 원문과 모델 ID·프롬프트 버전·샘플 구간·입력 해시를 함께 보존하며 입력 해시는 1단이 분석한 파일과 같아야 한다.
- 3단은 외부 유료 API로 오디오 구간을 보낸다. 기본값은 꺼짐이며 명시적으로 켰을 때만 동작한다. 2단 실패가 1단 결과 저장을 막지 않는다.
...
- 소비 시점 프롬프트에 519개 점수를 통째로 넣지 않는다. **가장 높은 점수의 0.5배를 임계값으로** 삼아 그 이상인 스타일만(상한 5개) 넣고, 보정되지 않은 상대 점수임을 함께 밝힌다. 절대 임계값을 쓰지 않는 이유는 점수 스케일이 곡·장르마다 달라서다 — 1위가 0.33인 곡이 0.97인 곡보다 덜 확실하다는 뜻이 아니다. 장르 목록이 짧아도 감정값과 자유 서술이 함께 가므로 정보 총량은 줄지 않는다. 원본 519개는 DB에 그대로 남아 재추론 없이 다시 뽑을 수 있다. 선택은 `runs.selectPromptStyles`가 단일 기준이며 완료 시 `maest_summary.prompt_styles`에 저장한다. 소비처가 임계값을 다시 정의하지 않는다.
