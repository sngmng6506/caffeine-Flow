...
- 2단 Audio LLM 프롬프트에 MAEST 결과·임베딩 벡터·고정 택소노미를 넣지 않는다. 두 단계는 독립이어야 앙상블이 되고, 결과가 갈리는 곡이 어려운 곡이라는 신호가 남는다. `audio_llm_raw`는 자유 서술 원문과 모델 ID·프롬프트 버전·샘플 구간·입력 해시를 함께 보존하며 입력 해시는 1단이 분석한 파일과 같아야 한다.
- 2단은 외부 유료 API로 오디오 구간을 보낸다. 기본값은 꺼짐이며 명시적으로 켰을 때만 동작한다. 2단 실패가 1단 결과 저장을 막지 않는다.
