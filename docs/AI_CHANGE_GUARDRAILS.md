...
- 아티스트 참고에는 artist_confirmed=true인 사람 라벨만 쓴다. 곡 검토 완료에서 아티스트 확인을 추정하지 않는다. 확인한 아티스트명의 정규화 키는 다른 곡 참고 조회에만 쓴다. 자동 동일인 판정이나 라벨 복사 근거로 사용하지 않는다.
- 사람 라벨은 `usage_scope`와 무관하게 라이브 판단의 Exact 재사용·프롬프트·동일 아티스트 참고·자동수락에 연결하지 않는다. 연결하려면 별도 평가와 계약 변경이 필요하며, 운영자 수동 참고 조회와는 구분한다.
- 운영자 인증과 `(cafe_id, recommendation_id)` 범위를 모두 확인한 AI 처리 이력만 검수한다.
- 전체 카페 라벨링 큐 조회는 `requireAdmin` 경계를 유지하며 사장님 JWT로 열지 않는다.
...
- **경로는 `EMOTION_LLM` 하나다** — Valence/Arousal + Audio LLM. 장르를 판단할 모델이 없으므로 분기할 모드도 없다. 장르 판단을 되살리려면 모델·검증·저장·소비처를 한 작업에서 함께 되살린다(근거는 `git log -S EMOTION_LLM`).
- `normalized.genre`는 **빈 배열**이어야 한다. `unknown`으로 채우지 않는다 — "모른다"와 "판단할 모델이 돌지 않았다"는 다르고, 소비처는 빈 배열일 때 장르 줄을 렌더하지 않는다. 보컬·악기도 장르에서 추측하지 않는다.
- **3단 프롬프트에 임베딩 벡터·고정 택소노미를 넣지 않는다.** 택소노미는 학습 분포 밖 음악의 정보를 지운다. **Valence/Arousal만 예외다** — `normalize_score`가 DEAM `[1,9]`를 `[0,1]`로 옮긴 보정된 값이라 0.6이 어디서나 같은 뜻이다. 3단과 필터 프롬프트 모두 척도를 함께 적어 숫자로 넣고 밴드 이름으로 바꾸지 않는다 — 밴드로 뭉치면 가까운 값의 차이가 사라진다. 두 곳의 형식(`0.60 (0.00 어두움 ~ 1.00 밝음)`)을 같이 유지한다.
...
- **구간은 2단보다 먼저 한 번 정하고 두 단계가 같은 구간을 듣는다.** `remote_analyze.py`가 계획하고 V/A 계산과 3단 호출에 함께 넘긴다. 각자 계획하면 프롬프트의 밝기·활력이 서술과 다른 구간의 값이 된다 — `describe()`가 스스로 계획하지 않는 이유다.
- **구간은 대표성 기준으로 고른다** — 곡 중앙 ±15초의 가장 큰 곳과 가장 많이 반복되는 구간. 곡 전체를 고르게 나눈 것이 아니므로 프롬프트에 "고르게 뽑았다"고 적지 않는다. 이 구성은 후렴 밖 특이 구간을 구조적으로 듣지 못한다(근거·측정은 [experiments/](../audio-analysis-worker/experiments/)).
...
- 전송 결과는 서버 확인 전 outbox에서 삭제하지 않는다. lease 인계 충돌(409)은 superseded, 서버가 페이로드를 거절했거나 작업이 사라진 경우(400·404·422)는 rejected 폴더로 옮겨 보존하고 로그에 남긴다. 다시 보내도 결과가 같은 항목을 재시도하지 않는다(이유는 [워커 README](../audio-analysis-worker/README.md#결과-보존과-장애-복구)). 일시적 실패(5xx 등)는 치우지 않고 재시도한다. 새 작업의 토큰으로 바꿔 제출하지 않으며, 서버 주소가 달라진 outbox는 전송하지 않는다.
- 관리자 재큐잉·재정규화는 인증과 작업/분석 버전을 확인하고 사람 라벨 및 원본 이력을 보존한다.
- 서버가 제출된 원본에서 정규화 라벨을 다시 확인한다. 워커가 보낸 라벨을 검증 없이 그대로 저장하지 않는다.
...
- 공유 DB에서 로컬 `migrate`를 실행하지 않는다. 배포 절차는 [DEVELOPMENT.md](DEVELOPMENT.md#마이그레이션)를 따른다.
