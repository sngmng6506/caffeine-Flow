...
- 소비 시점 프롬프트에 519개 점수를 통째로 넣지 않는다. **가장 높은 점수의 0.5배를 임계값으로** 삼아 그 이상인 스타일만(상한 5개) 넣고, 보정되지 않은 상대 점수임을 함께 밝힌다. 절대 임계값을 쓰지 않는 이유는 점수 스케일이 곡·장르마다 달라서다 — 1위가 0.33인 곡이 0.97인 곡보다 덜 확실하다는 뜻이 아니다. 장르 목록이 짧아도 감정값과 자유 서술이 함께 가므로 정보 총량은 줄지 않는다. 원본 519개는 DB에 그대로 남아 재추론 없이 다시 뽑을 수 있다.
