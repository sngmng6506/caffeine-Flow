...
- 곡 라벨 검토는 자동 서술이 곡과 맞는지만 묻는다. 판정은 `accurate`와 `inaccurate` 둘이며 늘리지 않는다 — 중간 선택지는 판단을 미루는 칸이 되어 어느 쪽으로도 쓰지 못하는 라벨만 쌓는다. `unclear`는 이미 저장된 옛 판정을 읽기 위해 조회에만 남기고 새로 만들지 않는다. 자동 라벨은 그대로 저장되고 판정은 최종 라벨의 `human_review_status`와 해당 분석의 `human_verdict`에 남는다. 직접 수정 이력은 `human_edited`로 구분한다. 매장 정책 검수는 곡 라벨과 달리 추천곡별로 유지하고 다른 매장·신청에 자동 복사하지 않는다.
...
- **Valence/Arousal은 예외로 3단 프롬프트에 넣는다(2026-09-17, 운영자 판단).** MAEST 점수와 달리 `normalize_score`가 DEAM 원본 척도 [1,9]를 [0,1]로 옮긴 보정된 값이라 0.6이 어디서나 같은 뜻이다. 척도를 함께 적어 숫자로 넣고, 밴드 이름(`어두움`/`중간`/`밝음`)으로 바꾸지 않는다 — 경계가 0.35/0.65라 valence 0.601과 arousal 0.641이 똑같이 `중간`이 되어 차이가 사라진다. 참고값임을 밝혀 3단이 들은 것과 어긋나면 들은 대로 쓰게 한다. 이 예외는 V/A 한정이며 위 항목의 나머지 금지는 그대로다.
- 3단은 외부 유료 API로 오디오 구간을 보낸다. 실행 여부와 시스템 프롬프트는 `audio_pipeline_settings`가 단일 기준이며 워커는 claim 응답으로 받는다. 프롬프트 본문은 `audio_prompt_revisions`에 추가만 되고 DB trigger가 수정·삭제를 막는다 — `audio_llm_raw`는 `prompt_version`만 보존하므로 이력이 없으면 옛 서술을 어떤 문장으로 만들었는지 되짚을 수 없다. 버전 문자열은 기본 `audio-llm-2`, 고친 경우 `custom-<sha256 앞 12자>`이며 서버와 워커가 같은 규칙으로 만든다. 워커 환경변수로 되돌리지 않는다 — 운영자가 Lab에서 끄지 못하게 된다. 3단 실패가 1단 결과 저장을 막지 않는다.
...
- **골드 라벨 비교와 false accept 측정 없이 연결했다(2026-09-10, 운영자 판단).** 판정 데이터가 0건인 상태에서 서술 정확도를 눈으로 확인한 것이 근거다. 측정 전까지 이 연결은 검증되지 않은 상태이며, 판정이 쌓이면 false accept를 확인하고 이 항목을 갱신한다.
