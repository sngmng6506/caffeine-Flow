...
- 자동 추천은 확신할 때만 칸을 채운다. 임계값 미만이면 값을 비우고 `missing:<칸>` 신호를 남긴다. 빈 칸은 사람이 고르지만 틀린 값은 눈으로 넘기는 검수를 그대로 통과한다.
- 신뢰도는 분류 헤드 확률이 있는 칸에만 붙인다. 템포·리듬처럼 BPM 구간으로 정하는 칸에 점수를 지어내지 않는다. 검수 정렬이 이 값을 신뢰한다.
- 분류 헤드의 클래스 순서를 코드에 하드코딩하지 않는다. 모델 `.json`의 `classes`를 읽어 이름으로 확률을 찾는다. 헤드마다 순서가 다르다.
- 자동 추천값에 `unknown`을 넣지 않는다. `unknown`은 사람이 "모르겠다"를 고르는 값이고, 모델이 확신하지 못하면 칸을 비운다.
- 분류 헤드가 없거나 Valence/Arousal 값이 없을 때 템포나 음량 휴리스틱으로 분위기를 추측해 채우지 않는다.
