...
- 곡 라벨 검토는 자동 서술이 곡과 맞는지만 묻는다. 판정은 `accurate`·`inaccurate`·`unclear` 세 가지이며 선택지를 늘리지 않는다 — 고르기 어려운 선택지는 아무거나 찍게 만들고 그렇게 만든 골드 라벨은 없느니만 못하다. 자동 라벨은 그대로 저장되고 판정만 `human_review_status`에 남는다. 매장 정책 검수는 곡 라벨과 달리 추천곡별로 유지하고 다른 매장·신청에 자동 복사하지 않는다.
...
- 3단은 외부 유료 API로 오디오 구간을 보낸다. 실행 여부는 `audio_pipeline_settings`가 단일 기준이며 워커는 claim 응답으로 받는다. 워커 환경변수로 되돌리지 않는다 — 운영자가 Lab에서 끄지 못하게 된다. 3단 실패가 1단 결과 저장을 막지 않는다.
