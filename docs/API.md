...
| POST | `/admin/audio-discoveries` | 🛡 | 수집 요청. body는 `{ source, query?, limit? }`. `soundcloud`는 검색 소스라 `query`가 필수이고(공백만이면 400), 나머지 소스는 보내도 무시하며 `query`는 null로 저장한다. 진도 커서는 `(source, query_key)`라 검색어마다 따로 남는다. 같은 소스가 대기 중이면 기존 요청을 200으로 돌려준다 |
