...
AI 필터의 ON/OFF는 대시보드 헤더의 **AI 자동수락** 버튼 하나로 조작한다.
켜면 필터를 통과한 곡만 사장님 앱이 자동으로 수락·재생하고, 끄면 필터 없이 모든 신청이 pending으로 들어와 수동으로 운영한다.
(서버 `music_filter_enabled`가 단일 원천이며, LLM accept → `pending` 상태 계약은 그대로다 — 자동 수락은 사장님 앱 클라이언트가 처리한다.)
...
매장 분위기 설명
필터 강도: 느슨하게 / 보통 / 엄격하게
AI 필터 테스트 (저장 없이 곡 URL로 판단 미리보기)
AI 오류 시 자동 거절 안내
```
...
`OPENROUTER_API_KEY`가 없는데 AI 필터가 켜져 있으면, 설계대로 신청곡은 `error_rejected` 처리되고 사장님 앱에 오류 알림이 표시된다.
