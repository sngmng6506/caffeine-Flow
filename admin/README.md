...
- 판단에는 정책 일치·메타데이터 충분 여부를, 곡에는 확인한 아티스트·버전·템포·분위기·사운드·리듬·보컬·장르·메모를 구조화해 기록한다. 사람 라벨은 큐 상태와 LLM 판단을 바꾸지 않는다.
- 미검수 카드는 AI 결과를 숨기고 곡·정책을 먼저 보여준다. 사람 정답 저장 후에만 공개해 라벨 편향을 줄인다.
- 헤더에서 관리자 세션을 유지한 채 [`/filter-lab`](../music-filter-lab/README.md)과 [`/labeling-lab`](../music-labeling-lab/README.md)으로 이동한다. 라벨링 랩은 정책 검수와 곡 라벨이 모두 저장된 항목만 완료로 센다.
