...
AI 도구(Claude Code · Codex · Gemini 등)로 이 저장소를 작업할 때의 규칙. 사람 기여자도 동일하게 따른다.

이 파일은 **도구 무관 계약(contract)**이다. 특정 도구 설정(CLAUDE.md, GEMINI.md, .cursor/rules 등)은 이 파일을 참조하는 얇은 어댑터로 유지한다.
...
2. **설계** — 선택지가 있으면 대안과 트레이드오프를 짧게 정리하고 진행. 파괴적 변경(스키마·삭제)은 먼저 확인받는다.
3. **구현** — 논리 단위로 작게. 각 단계마다 검증(구문 검사·테스트).
...
- **마이그레이션·DB 로직은 실제 스키마로 검증한다.** 과거 `MIN(uuid)` 사고는 정수 PK로 가정한 로컬 테스트에서 통과해 놓친 것 — 실제 컬럼 타입을 확인할 것.

### 공유 DB 보호
- Railway 등 공유 DB에 로컬에서 `db:migrate`를 실행하지 않는다. 배포 startCommand가 자동 실행한다.
- 마이그레이션은 up/down 모두 구현. 파괴적 작업은 삭제 대신 상태 변경(예: `rejected` 마킹)으로 데이터를 보존.

### owner 번들 동기화
- `owner/src` 수정 후에는 `cd owner && npm run build`로 `server/public/owner/`를 리빌드해 커밋한다. 이 번들은 Railway fallback으로 커밋되므로 소스와 어긋나면 안 된다.
...
- **라우트 마운트 순서** — owner 라우터가 public보다 먼저 마운트돼야 인증 핸들러가 경로 매치를 가져간다. `server/app.js` 순서를 바꾸지 말 것.
- **recommendations.id는 UUID** — 집계에 `MIN/MAX(id)` 금지. `ROW_NUMBER() OVER (... ORDER BY)` 사용.
- **KST 기준 통계** — 날짜 경계·시간대 집계는 `utils/kst.js`를 쓴다. UTC 자정을 직접 계산하지 말 것.
- **상태 전이** — 종료 상태(played/skipped/rejected)에서 되돌리는 전이는 서비스 레이어에서 막혀 있다.
- **app.js / server.js 분리** — 테스트가 `app.js`를 import한다. 앱 조립 로직을 server.js로 되돌리지 말 것.
- **한국어 커밋·주석** — 코드 주석과 커밋은 한국어. 코드 식별자는 영어.
