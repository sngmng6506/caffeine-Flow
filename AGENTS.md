...
- **왜 그렇게 돼 있는지는 `git log`에서 찾는다.** 커밋이 Why/Decision을 담으므로 사고 기록과 판단 근거는 문서에 복사하지 않는다. `git log -S '<코드 조각>' -- <파일>`로 그 줄을 도입한 커밋과 이유를 찾는다.
- 문서에는 **코드에도 없고 `git log -S`로도 못 찾는 것**만 남긴다. 파일을 가로지르는 불변식(양쪽 코드 어디에도 안 적힌 것)과 코드에 들어가지 않은 측정 수치가 그것이다.
...
| 오디오 분석 워커, 자동 라벨링 작업 큐, 구간 선택·정규화 | [audio-analysis-worker/README.md](audio-analysis-worker/README.md) |
