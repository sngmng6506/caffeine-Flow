...
export default function CafeProfileSettings({ cafe, onCafePatch }) {
  ...
  async function saveNotice(value) {
    ...
    try {
      ...
      setMessage({ tone: 'success', text: notice ? '매장 공지를 적용했어요.' : '매장 공지를 삭제했어요.' });
    } catch (error) {
      console.error(error);
      setMessage({ tone: 'error', text: error.message || '매장 공지를 적용하지 못했어요. 다시 시도해 주세요.' });
    } finally {
