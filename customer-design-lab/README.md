...
자료조사(야미카와이=파스텔 핑크 지배 + 의료 모티프 / 사이버펑크=다크 베이스 + 네온 글로우·글리치)를 바탕으로, 둘의 접점인 '핑크'를 축으로 융합한 방향이다.

- M1 · Yami Pulse: 다크 위 핫핑크 지배 + 시안 헤일로 글로우, 하트 모티프 (핵심)
- M2 · Sick Sweet: 핑크/라벤더/베이비블루의 파스텔 병약 스위트, 부드러운 라운드
- M3 · Glitch Heart: 네온 마젠타×시안, 클립 코너와 글리치 크로매틱 타이포 (사이버펑크 우위)

### 현재 앱 베이스 기반 B1~B2

새 팔레트로 갈아엎지 않고, 지금 쓰는 정체성(네온 라임·순검정·핫핑크·각진 구조)을 베이스로 사이버멘헤라를 얹은 방향. 이미 있는 핫핑크·시안을 살려 글로우·♡·글리치만 가미한다.

- B1 · Signal Heart: 라임 주역 유지 + 핑크 글로우·♡·글리치 (현재 정체성 최대 유지)
- B2 · Signal Heart Pink: 핑크를 전면 주역으로, 라임/시안은 액센트로 강등 (멘헤라 우위)

### 의료 디자인 언어 V1

멘헤라를 이모지(♡)·글리치 같은 장치가 아니라 실제 의료 디자인 언어로 표현한 방향. 현재 앱 베이스(라임·검정·핑크)는 유지한다.

- V1 · Ward: 봉합선(핑크 대시=꿰맨 자국)·의료 크로스(선으로 그린 ＋)·캡슐(알약) 배지

상단에서 전체/A~Y·E1~E4·M1~M3·B1~B2·V1 시안을 선택하고 390px·480px 너비를 전환할 수 있다. 최종안이 정해지기 전에는 이 폴더의 스타일을 운영 customer/src에 반영하지 않는다.
