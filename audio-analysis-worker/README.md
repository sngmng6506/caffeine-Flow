...
4. 서버 설정이 켜져 있으면 CPU 분석과 동시에 곡에서 고르게 뽑은 구간을 오디오 입력 LLM에 보내 무드·악기·보컬을 자유 서술로 받는다([3단 Audio LLM](#3단-audio-llm)). 두 결과가 모인 뒤 제출한다. CPU 분석이 실패해도 이미 시작한 외부 호출의 비용은 발생할 수 있다.
...
자식 초기화와 곡 분석은 각각 600초 제한이다. 추론 오류·시간 초과·자식 종료 시 해당 자식을 폐기하고 새 자식이 모델을 검증한 뒤 다음 claim을 받는다. 부모는 TensorFlow를 로드하지 않는다. 감정 모델이 없는 상태도 자식 수명 동안 유지하므로 모델 파일을 설치·교체하면 서비스를 재시작한다. 모델 파라미터·샘플레이트·결과 스키마는 동일하다. 실제 성능과 예측기 재사용의 수치 일치는 미니PC에서 같은 음원을 단일 실행과 연속 실행으로 비교해 확인한다.
...
- **1단 결과를 프롬프트에 넣지 않는다.** `audio_llm.py`에는 장르를 받을 인자 자체가 없다.
