...
사람 검토는 자동 서술이 곡과 맞는지만 묻는다. 판정은 `맞음`과 `틀림` 둘이고 자동 라벨 값 자체는 바뀌지 않는다. `맞음`만 `label_source`를 `human`으로 승격하며, `틀림`은 자동 라벨로 남겨 재분석·재정규화가 그 곡을 계속 갱신할 수 있게 한다. 승격하면 두 작업이 `automatic`인 것만 건드리므로 틀렸다고 표시한 곡이 영영 고쳐지지 않는다. 실제 아티스트 확인은 별도 체크박스이며 업로더 채널명을 정답으로 승격하지 않는다. 화면과 단축키는 [라벨링 랩 README](../music-labeling-lab/README.md)에 있다.
